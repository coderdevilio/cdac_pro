...
const Medium = () => {
  ...
  const mediumQuestionsCollectionRef =collection(db,'MediumQuestions')

  const getEasyQuestions = async () =>{

    try {
      const data = await getDocs(mediumQuestionsCollectionRef);
    ...
    const onSubmitEasyQuestion = async () =>{
      ...
      try {
        await addDoc(mediumQuestionsCollectionRef,{
          Problem:problem,
